...
void main() {
  ...
  final List<Chord> _baseChordProgression = [
    Chord.parse('F'),
    Chord.parse('G'),
    Chord.parse('C'),
  ];
  ...
  final List<Scale> _possibleScales = matchChordsWithKey(_baseChordProgression);
...
List<Scale> matchChordsWithKey(List<Chord> chords) {
  return matchChordNamesWithKey(
      chords.map<String>((Chord chord) => chord.getCommonName()).toList());
}
